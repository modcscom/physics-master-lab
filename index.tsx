...
1. 题目应多样化，包含：
   - 纯文字计算题
   - 带图片描述的实验探究题（用 [图片描述: ...] 格式描述实验装置或物理场景）
   - 图表分析题（用 [图表描述: ...] 格式描述坐标图、数据表等）
   - 力学示意图题（用 [图片描述: ...] 格式描述受力分析图、运动轨迹图等）
   - 电路图题（用 [图片描述: ...] 格式描述电路连接图）
   
2. 每份试卷应包含至少 2-3 道带图片/图表的题目，例如：
   - 浮力题：描述物体在水中的浮沉状态图
   - 滑轮组题：描述滑轮组装置图
   - 电路题：描述电路连接图
   - 光学题：描述光路图
...
3. 图片描述格式：[图片描述: 详细描述图片内容，如"一个质量为2kg的木块漂浮在水面上，露出水面的体积约占总体积的1/4"]
`;
